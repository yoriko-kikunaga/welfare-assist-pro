...
export type AsOfBasis = 'start' | 'end';

/**
 * 指定月（'YYYY-MM'）に有効だった属性値を返す（月度ページで「その月の状態」を表示するため）。
 *
 * - 基準日: basis='end'（既定）= 月末。その月内に起きた変更も反映（例: 5/15に介護度変更 → 5月は新しい値）。
 *           basis='start' = 月初。月途中の変更は翌月から反映。
 ...
 */
export function getClientAttributeAsOf(client: Client, field: string, month: string, basis: AsOfBasis = 'end'): string {
  ...
  const ref = basis === 'start' ? `${month}-01` : `${month}-${String(lastDay).padStart(2, '0')}`;
