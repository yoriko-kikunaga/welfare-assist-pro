...
callout(s, 0.7, 5.7, 11.9, 0.95,
        "⚠️ 自動保存はありません",
        "入力したら必ず「保存する」ボタンを押してください。未保存の間は「未保存の変更があります」と表示されます。",
        warn=True)
...
steps = [
    ...
    ("3", "内容・詳細を記入して保存",
     "議事録内容（テキストエリア）を入力し、右上の「保存する」を押します。自動保存はありません。"),
]
x = 0.7
for n, h, b in steps:
...
qas = [
    ("入力を間違えました。保存前に元に戻せますか？",
     "自動保存はないので、保存する前なら「キャンセル」で元の内容に戻せます。保存後に間違いに気づいた場合は、直して再度「保存する」を押してください。"),
    ("AIサマリーが生成されません",
