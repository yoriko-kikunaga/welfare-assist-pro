...
callout(s, 0.7, 4.3, 11.9, 1.0,
        "⚠️ 新しいカードは初めどれもチェックされていません",
        "必ず、あてはまるものにチェックを入れてください（複数可）。チェックしないまま保存すると空欄でシートに出力されます。",
        warn=True)
...
qa = [
    ("追加したのに保存できない／消えた", "黄色い「入力中」のままかも。右上の「保存する」で確定。キャンセルで破棄。自動保存はないので、保存せず移動しようとした時に出る確認ダイアログで「保存して移動」を選べば安心です。"),
