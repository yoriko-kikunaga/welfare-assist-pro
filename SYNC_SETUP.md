...
### 🛑 安全ガード作動: "利用者数が異常に少ないためインポートを中断しました"

**原因:** インポート結果の利用者数が異常に少ない（絶対下限8,000件未満、または前回比90%未満）。
スプレッドシート/Kintoneの一時的な読み込み障害などで発生する。

**動作:** `importSpreadsheetData.cjs` が `clients.json` を**書き換えずに** `exit(1)` で中断する。
ワークフローも失敗し、**壊れたデータがコミット・デプロイされない**（2026-06-01の本番データ破損事故の再発防止）。

**解決策:**
1. まずはログを確認し、読み込み障害なら再実行（多くは一時障害で次回成功する）。
2. 事業所統廃合などで**正当に大量削減**された場合のみ、手動で `--force` を付けて実行：
   ```bash
   node importSpreadsheetData.cjs --force
   ```
