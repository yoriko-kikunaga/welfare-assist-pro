            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 animate-fade-in-up">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* 施設契約情報（Kintone由来・施設への入居/退去。レンタルの新規/解約とは別系統） */}
                  <div className="md:col-span-2">
                    <h3 className="text-lg font-bold text-gray-800 border-l-4 border-teal-500 pl-3 mb-4">
                      施設契約情報
                      <span className="ml-2 text-xs font-normal text-gray-400">（Kintone連携・施設への入居/退去）</span>
                    </h3>
                    {(() => {
                      const { facilityPairs, unpairedFacilityMoveOuts } = buildFacilityContractPairs(editedClient.changeRecords);
                      if (facilityPairs.length === 0 && unpairedFacilityMoveOuts.length === 0) {
                        return (
                          <div className="text-sm text-gray-400 bg-gray-50 border border-dashed border-gray-200 rounded-lg p-4">
                            施設入居・退去の情報はありません（施設入居者のみ表示）
                          </div>
                        );
                      }
                      return (
                  ...
                  {(() => {
                      ...
                      return (
                          <>
                              ...
                              {/* ペアになっていない退院レコード（入院とペアにならなかった単独の退院） */}
                              {unpairedDischarges.map((record) => (
                                  <div key={record.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                      <div className="p-4 bg-gray-100 flex justify-between items-center">
                                          <h4 className="text-sm font-bold text-gray-800">退院（単独）</h4>
                                          <span className="text-xs text-gray-400">{record.recordDate} | ID: {record.id}</span>
                                      </div>
