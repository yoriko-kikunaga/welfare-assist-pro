...
- **データ**: 9209件の利用者（Google Sheets + Kintone連携）
...
| Client | `clientName`, `office`, `facilityName`, `roomNumber`, `careSupportOffice`, `careManager`, `careLevel`, `copayRate`, `insuranceCardStatus`, `burdenProportionCertificateStatus`, `paymentType`, `billingCategory`(請求区分), `kaipokeRegistrationStatus`, `location`(在宅区分), `keyPerson`, `medicalHistory`, `isWelfareEquipmentUser`, `insuranceRentalBillingTotal` |
...
**変更履歴（`attributeHistory`）**: 基本情報の追跡14項目（事業所/施設名/部屋番号/在宅区分/福祉用具利用者/レセプト対象/居宅事業所/担当CM/要介護度/負担割合/被保険者証/負担割合証/支払い区分/請求区分）の変更を `clientEdits.attributeHistory[]`（`{id,field,value,effectiveFrom(手動日付),recordedAt,note?}`）に追記保持。`ClientDetail.tsx` で変更を即検知（select/checkは onChange、textは onBlur）→実効日入力ダイアログ→追記。各項目横の🕐でタイムライン表示。**編集モードでは🕐モーダル下部の「履歴を追加」フォームから過去の履歴を手動追加可**（値・実効日[必須]・備考。`addManualHistoryEntry`）。誤記録は同モーダルで削除可。`firestoreAdmin.cjs` の夜間mergeでも保持。初回記録時は「変更前の値」を `effectiveFrom:''`（記録開始前）のベースラインとして併せて追記（過去月を正しく引くため）。
> **月度の状態表示**: `src/utils/attributeHistory.ts: getClientAttributeAsOf(client, field, 'YYYY-MM', basis)` が「その月に有効だった値」を返す（履歴なければ現在値）。`basis`=`'end'`(既定,月末)/`'start'`(月初)。現状の適用先は**レセプトチェックのみ**（`receiptCheckService.ts` の `refreshItemsFromClients`/`generateReceiptCheckFromClients` で 事業所・在宅区分・居宅事業所・生保判定 に使用）。基準日トグルは ReceiptCheckPage 上部（localStorage `receiptCheckAsOfBasis` で保持）。
...
- **セクション表示順**: 入力中（amber）→ 入院・退院情報 → 退院（単独） → その他（変更あり/その他） → レンタル契約情報（`新規`/`解約`・介護保険レンタル由来） → 解約（単独）
- **単独レコードの表示バグ2件を修正（2026-09-10）**: ①`unpairedCancels.map`が「解約（単独）」セクション内で2箇所に重複描画されており、同じ解約レコードが画面に2回表示されていた（片方は`請求開始日（新規）`という誤ったフィールドを表示する不具合版）。誤り側を削除し、正しい`請求停止日（解約）`を表示する側のみ残した。②`unpairedDischarges`（入院とペアにならなかった単独の退院レコード）が計算されているのに描画箇所が存在せず、該当レコードが一覧から見えなくなっていた。「解約（単独）」と同じ構成で「退院（単独）」セクションを新規追加（日付項目は`請求開始日（退院）`＝`billingStartDateDischarge`）。両セクションとも入院/退院カードと同様の理由でリマインダーは対象外。
- **施設契約情報は基本情報タブへ移動（2026-06-22）**: Kintoneアプリ197（入居・退去）由来の施設入退去ペアは Tab4 から外し、**基本情報タブの生年月日・性別の下**に「施設契約情報」セクションとして表示（編集可）。ペアリングは module 関数 `buildFacilityContractPairs(records)`（ClientDetail.tsx 冒頭）。
  - **施設レコードの判定（重要）**: App197 のクエリは「2025-11-01以降」のみ取得するため、それ以前の入居レコードは **infoType が `新規`/`解約` のまま id が `kintone-197-movein/moveout-*`** で恒久的に残り、2025-11以降の新規分のみ infoType `施設入居新規`/`施設入居解約` になる（混在）。そのため施設判定は **infoType（施設入居新規/解約）または id プレフィックス（`kintone-197-movein/moveout-`）** の OR で行う（`isFacilityMoveIn`/`isFacilityMoveOut`/`isFacilityRecord`）。
  - **入居→退去のペアリング**: ①**同じKintoneレコード番号**（`movein-X`↔`moveout-X`・`facilityRecordNumber()` で id 末尾を抽出）を最優先→②日付ベース（入居日以降で最も早い退去）。movein/moveout は同一App197レコード由来で番号が一致するため①で確実に正しくペアになる（日付ベース単独だと誤マッチする）。※`kintone-*` レコードはマージでベース優先のため手動ペア編集は同期で消える＝自動修正方式を採用（2026-06-22）。
  - **レンタル契約情報**（Tab4・`新規`/`解約` のうち施設でない手動入力分）は `!isFacilityRecord(r)` で施設レコードを除外。
  - ※ `receiptCheckService` は従来どおり `infoType === '新規'`/`'解約'` を参照（古い施設入居=新規はレセプト母集団に含まれる既存挙動・2026-05-12以降の新規分は施設入居新規のため対象外）。表示の再分類はデータ・他サービスに影響しない。
...
- **利用区分チェックボックス（2026-06-22・初期値は2026-08-26に変更）**: 各変更レコードフォーム（入力中／変更あり・その他／レンタル契約情報の新規・解約）に「利用区分」を**チェックボックス（自費レンタル/介護保険レンタル/販売・複数選択可）**で追加（`UsageCategoryCheckboxes`）。値は `ClientChangeRecord.usageCategory`（型は`string`に拡張）に**「・」連結**で保持。スプレッドシートG列「利用区分」は元々 `record.usageCategory` を出力しているため自動反映。※入院/退院・施設入居(Kintone由来)カードには付けない（編集が同期で戻るため）。**シート同期は追記専用**＝既存行のusageCategoryは更新されない。新規レコードのみ正しいG列で追記される。
  - **新規追加レコードの初期値は未選択（`''`）**（2026-08-26変更・`handleAddChangeRecord`）。以前は`介護保険レンタル`固定だったため、自費レンタル・販売の新規登録でも常に介護保険レンタルにチェックが入り、追記専用の同期の仕組み上あとから直しても反映されない問題があった。
- **事業所の決め打ちバグ修正（2026-08-26）**: `handleAddChangeRecord`が新規レコードの`office`を`'鹿児島（ACG）'`に決め打ちしており、フォーム上の「事業所（基本情報から参照）」欄は`editedClient.office`を表示するだけで実際のレコードには反映されていなかった。福岡（Lichi）の利用者でも変更情報・スプレッドシート出力の事業所が常にACGになる不具合があり、`office: editedClient.office || '鹿児島（ACG）'`に修正して解消。
- **リマインダー（あり/なし）ラジオボタン（2026-09-10）**: 議事録タブ（`MeetingRecord.reminder`）と同じ仕様を変更レコードにも追加。`ClientChangeRecord.reminder?: ReminderStatus`、UIは共通コンポーネント`ReminderRadio`（`UsageCategoryCheckboxes`と同じ並び）。配置は入力中／変更あり・その他・デモ／レンタル契約情報の新規・解約（ペア/単独とも）の**5箇所**（利用区分チェックボックスと同じ「Kintone由来の入院/退院カードには付けない」方針を踏襲）。新規レコードの初期値は`'なし'`。
  - **スプレッドシート反映**: システム管理19列（A:S）や既存のスタッフ運用列（T:W＝デモ/デモ進捗確認/自費/確認）を一切動かさず、さらに後ろの**X列**に単独で追記する方式（`syncChangeRecordsToSheets`）。A:S列の更新判定とは独立して「X列の既存値とレコードのreminderが違う」場合のみ書き込む。
  - **列追加の注意（ハマりどころ）**: シートのグリッド列数が既存範囲ぴったりだと新列への書き込みで`exceeds grid limits`エラーになるため事前に`insertDimension`で列を追加する必要がある。ただし`appendDimension`は**隣接列（確認列＝チェックボックス）の書式・データ検証を継承してしまい、新列の全行が誤ってFALSE表示になる事故**が実際に発生した（`insertDimension` + `inheritFromBefore: false`で回避）。
  - **Y列「リマインダーチェック」（2026-09-10・ユーザーが手動追加）**: X列（システム管理・リマインダー本体）のさらに後ろに、スタッフがチェックボックス列を独自追加した運用列。T:W列と同じ扱い＝`syncChangeRecordsToSheets`は一切書き込まない（現状X列までしか書かないため対応不要・確認済み）。今後X列より後ろに新しい列を追加する場合は、グリッド拡張判定（`REMINDER_COLUMN_INDEX`）がY列以降を巻き込まないか要確認。
...
### 自動保存の廃止と離脱ガード（ClientDetail・2026-08-26）

**背景**: 変更情報のスプレッドシート同期（追記専用・既存行は更新不可）が1.2秒デバウンスの自動保存と組み合わさり、「情報を追加」直後の空欄状態（記録者・特記が未入力）のまま4秒後に同期されてしまい、あとから入力し直しても二度と反映されない不具合が発生していた（上記の事業所決め打ち・利用区分初期値固定も同根の問題）。

**対応**:
- **ClientDetail全体の自動保存（デバウンス）を廃止**。保存はヘッダーの「保存する」ボタン押下時のみ行う（`handleSave`）。変更情報・議事録のスプレッドシート同期（4秒デバウンス）もこの保存完了後にのみ発火する（`scheduleChangeRecordsSync`/`scheduleMeetingsSync`は変更なし、呼び出し元を自動保存effectからhandleSaveに一本化）。
- **離脱ガード**（保存必須化に伴う変更喪失対策・3点セット）:
  1. **未保存バッジ**: `isDirty`（`editedClient`と`client`propのJSON差分）が真の間、ヘッダーに「未保存の変更があります」を常時表示
  2. **`beforeunload`警告**: 未保存の間はブラウザのタブを閉じる/リロード時に標準の離脱確認ダイアログを表示
...
- **CSV出力（2026-06-22）**: ヘッダー右の「CSV出力」ボタンで**表示中（事業所フィルター適用後＝`welfareUsers`）の全利用者**を出力（選択中サブグループに依存しない）。列: 事業所/施設名(空は在宅)/居室番号/あおぞらID/氏名/フリガナ/要介護度/負担割合/請求区分/利用区分/福祉用具件数/居宅介護支援事業所/担当CM/生活保護。利用区分はStatus別タブと同じ判定（`statusLabel`）。UTF-8 BOM付き・事業所→施設→カナ順ソート。事業所別タブのときは全事業所が対象。
...
- **入居施設名のコンボボックス（2026-06-22）**: Tab1の「入居施設名」は独自コンポーネント `components/FacilityNameSelect.tsx`（候補から選択も手入力も可）。**フォーカス／▼クリックで候補を全件表示**（既存値で絞り込まない・`query`空＝全件）、**文字を打つとその文字で絞り込み**。当初 `<datalist>` で実装したが「既存値に一致する候補だけ出て全件見えない」ため独自実装に変更（2026-06-22 同日）。候補マスターは `src/constants/facilityMaster.ts` の `FACILITY_NAME_OPTIONS`（**手動管理**・並び順=表示順）。テキスト入力のままなので🕐履歴のonFocus/onBlur検知も従来どおり動く（option選択は onMouseDown preventDefault でフォーカス維持→離脱時に履歴検知）。マスター初期値は登録済み施設名を精査（有料/あおぞら表記は基本名に統合・野芥は数字なし・他社施設アルファリビングは除外・笑苑は梅ヶ丘に統合・うらら→介付有料うらら/GHうらら）。GHは今後マスターに追記予定。マスターを変えても既存利用者のfacilityNameデータは不変（候補に出なくなるだけ）。
- **居宅介護支援事業所のコンボボックス（2026-08-26）**: Tab1「ケアマネージャー情報」の「居宅介護支援事業所」も `FacilityNameSelect.tsx` を流用（候補から選択も手入力も可、入居施設名と同じUI）。コンポーネント名は`FacilityNameSelect`のままだが元々`options`をpropで受け取る汎用実装のため、汎用コンポーネントとして両方の項目で共用（`toggleAriaLabel`propで▼ボタンのaria-labelを用途別に指定）。候補マスターは `src/constants/careSupportOfficeMaster.ts` の `CARE_SUPPORT_OFFICE_OPTIONS`（**手動管理**・並び順=表示順、初期値: あおぞら介護ステーション介護支援事業所／あおぞら介護ステーション福岡介護支援事業所）。Tab4の「居宅介護支援事業所（基本情報から参照）」は従来どおり読み取り専用のまま変更なし。
- **施設名の表記ゆれ正規化（2026-06-22）**: `facilityNormalize.cjs`（ルート）の `FACILITY_NAME_NORMALIZE` マップ＋`normalizeFacilityName()` で表記ゆれ→正式名に統合。`importSpreadsheetData.cjs` が施設利用者シートの `facilityName`(row[5]) 取込時に適用（毎回の同期で自動正規化）。現行 `clients.json` にも一度適用済み（91名）。マップ: あおぞら〇〇→〇〇 / 〇〇・有料・有料・〇〇→基本名 / 野芥1→野芥 / アルファリビング鹿児島東千石→空(在宅)。**笑苑・うらら1・うらら2 は対象外**（うららは利用者ごとに介付有料うらら/GHうららへ手動で寄せる運用）。新しい表記ゆれはマップに追記。
...
- **変更情報スプレッドシート同期**: `syncChangeRecordsToSheets` Cloud Function（スプレッドシートID: `1E3jT222WbUYs2s_TXsme3HpmNqWG8fKHxqgQFBrEcQU`）
  - **追記＋更新モード（2026-08-26〜）**: 1列目`レコードID`をキーに、①未書き込み → 末尾に追記、②既存だがA:S列（システム管理の19列）の内容がFirestore側と異なる → 該当行をA:S列のみ`values.batchUpdate`で上書き更新、③内容が同じ → 何もしない、の3パターンに振り分け
    - **更新はA:S列に厳密限定**（範囲を`A{行}:S{行}`で指定）。T列以降（デモ/デモ進捗確認/自費/確認などスタッフが手動追加した運用列）は一切触らない
    - 既存ID・既存値は**A:S列を全行取得**して`レコードID→(行番号,値)`のマップを作成（ヘッダー位置に依存しない・`'レコードID'`ラベルと空行は除外）。ヘッダー復元が発生した場合は行ズレが起きるため、復元処理の後に必ず再取得してから行番号を確定する
    - **旧仕様（〜2026-08-25）は追記専用**で、一度同期済みのレコードを後日編集しても該当行は更新されなかった（記録者・特記の追記などが反映されない不具合の根本原因）。2026-08-26のデプロイ時に初回アップサートを実行し、Firestoreとシート内容が乖離していた過去分414件を一括更新済み
  - **初回同期**: ヘッダー行 + 全レコードを書き込み後、ヘッダー行を太字・青背景にフォーマット＋1行目を固定（frozenRowCount）
  - **ヘッダー自動復元**: 同期実行時に1行目が`レコードID`でない場合（手動ソート等でヘッダーが移動）、先頭にヘッダー行を挿入して書式・固定を再設定する
  - **注意（運用）**: この出力シートは**ヘッダー行を含めた範囲ごと並べ替えない**こと（ヘッダーが中段に移動し表示崩れ＋重複の原因）。並べ替えはフィルター表示を使う。1行目は固定済み
  - **日付フィルタ**: `CHANGE_RECORDS_START_DATE = '2026-02-01'`以降のレコードのみ出力（過去の履歴データを除外）
  - **除外リスト**: スプレッドシート内「除外リスト」シート（A列: レコードID）に記載されたレコードは同期対象外（追記・更新とも常にスキップ）。シートがない場合は初回同期時に自動作成。スプレッドシートから行を削除した場合はレコードIDをここに貼り付けることで再追記を防止できる
  ...
  - **「通し」列の手動追加による列ズレ事故と復旧（2026-09-08〜09-10）**: スタッフがB列に「通し」列を独自追加（真の列挿入ではなく、新しい行にだけ通し番号を入れる運用）したため、ヘッダー上の列位置と実データの列位置が食い違う状態が発生。当時の内訳は全1535行中1088行がヘッダーと不一致（実データはB列以降が1列分ずれずに残っていた側）、446行は通し番号込みで正しく一致という混在状態だった。
    - **発覚の経緯**: 「変更情報タブにリマインダーのラジオボタンを追加してほしい、あわせてスプレッドシートのヘッダーも確認してほしい」という依頼を受けた調査中に発覚。
    - **一次対応（ユーザーが手動でB列を削除）**: 通し列の追加者が別スタッフだったと判明したため、ユーザー自身がB列を手動削除。これで一部行（当時「正しく一致」していた446行相当）は復元されたが、**元々ヘッダーと不一致だった側（297行、主にKintone自動連携の入退院・施設入退去レコード）は今度は逆方向にズレたまま**残った。さらに、列ズレ発見時に**スタッフが「特記」欄に直接追記していた7行の続報メモ**（例:「センサーデモ」→「センサーデモ→8/17レンタル開始」）が、この手動削除の過程で失われた。
    - **復旧**: 削除前に取得していたバックアップ（`change_records_sheet_backup_20260909.json`、ローカル一時領域）を正として、297行中294行を自動修正（あおぞらID・利用者名・事業所などを正しい列へ）、7行の続報メモをバックアップから復元。残る3行は**同一レコードIDが2行存在する重複**（うち1件は内容も微妙に異なる＝自動保存タイミングのズレによる孤立行、2件はKintone自動連携の完全重複）と判明したため、列ズレ側（古い/不要な方）を削除して解消。
    - **教訓**: このスプレッドシートは既存の19列（A:S、システム管理）＋T〜W列（スタッフ運用列）という構成が前提。**既存列の間に列を挿入すると、この前提が壊れて同様の事故が再発する**。新しい列が必要な場合は既存列の**一番後ろ**（現在はX=リマインダー、Y=リマインダーチェックまで使用）に追加すること。
