...
- **入居施設名のコンボボックス（2026-06-22）**: Tab1の「入居施設名」は `<input list>`＋`<datalist>` の候補つきテキスト（候補から選択も手入力も可）。候補マスターは `src/constants/facilityMaster.ts` の `FACILITY_NAME_OPTIONS`（**手動管理**・並び順=表示順）。入力欄のままなので🕐履歴のonBlur検知も従来どおり動く。マスター初期値は登録済み施設名を精査（有料/あおぞら表記は基本名に統合・野芥は数字なし・他社施設アルファリビングは除外・笑苑は梅ヶ丘に統合・うらら→介付有料うらら/GHうらら）。GHは今後マスターに追記予定。マスターを変えても既存利用者のfacilityNameデータは不変（候補に出なくなるだけ）。
