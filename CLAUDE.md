...
**注意（残課題）**: スプレッドシート同期が追記専用である点は変わらないため、**一度保存・同期済みのレコードを後日また編集して保存し直しても、シート側の該当行は更新されない**（利用区分の修正・特記の追記等）。追記専用→ID一致行は上書き更新、への変更は未着手。
...
- **変更情報スプレッドシート同期**: `syncChangeRecordsToSheets` Cloud Function（スプレッドシートID: `1E3jT222WbUYs2s_TXsme3HpmNqWG8fKHxqgQFBrEcQU`）
  - **追記モード**: 1列目`レコードID`をキーに既存行をスキップし、新規レコードのみ末尾に追記（上書き不可）
    - 既存ID判定は**全行から収集**（ヘッダー位置に依存しない・`'レコードID'`ラベルと空行は除外）。手動ソートでヘッダーが動いても重複追記しない
  - **初回同期**: ヘッダー行 + 全レコードを書き込み後、ヘッダー行を太字・青背景にフォーマット＋1行目を固定（frozenRowCount）
  - **ヘッダー自動復元**: 2回目以降の同期で1行目が`レコードID`でない場合（手動ソート等でヘッダーが移動）、先頭にヘッダー行を挿入して書式・固定を再設定する
  ...
  - **列構成（19列）**: レコードID / 入力日 / あおぞらID / 利用者名 / 施設名 / 情報種別 / 利用区分 / 請求開始日（新規）/ 請求停止日（入院）/ 請求開始日（退院）/ 請求停止日（解約）/ デモ開始日 / デモ終了日 / データ連携日 / 卸会社停止連絡 / 卸会社再開連絡 / 記録者 / 事業所 / 特記
