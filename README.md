...
- **データ品質向上**（自動インポート統合済み）:
  ...
  - **修正履歴**（2025-12-24）:
    ...
    - **重要**: データ読み込み方法の改善（2025-12-24）:
      ...
      - データ更新後は`cp clients.json public/assets/clients.json`でpublicフォルダにコピーが必要
...
| 事業所 | ドロップダウン | 鹿児島（ACG） / 福岡（Lichi）。他のタブの事業所項目はここから参照される（読み取り専用） |
| 氏名 | テキスト | 利用者の姓名 |
| フリガナ | テキスト | カタカナ表記 |
...
| 項目名 | 入力形式 | 選択肢 |
|--------|----------|--------|
| 事業所 | テキスト（読み取り専用） | 基本情報タブから自動参照（鹿児島（ACG） / 福岡（Lichi）） |
...
**インテリジェントグルーピング表示（2026-01-08更新）:**

レコードは関連する情報をペアで表示し、サービスのライフサイクルを視覚的に把握できます：

1. **入院・退院ペア（オレンジヘッダー）**: 入院と退院を横並びで1つのカードに表示
   - **左側（赤背景）**: 入院（サービス停止）情報
   - **右側（緑背景）**: 退院（サービス再開）情報
   - **ペアリングロジック**: 入院日以降の最も早い退院日を自動マッチング
   - 退院情報がない場合は「退院情報なし」プレースホルダーを表示
   - 入院日の新しい順にソート

2. **新規・解約ペア（紫ヘッダー）**: 新規契約と解約を横並びで1つのカードに表示
   - **左側（青背景）**: 新規契約情報
   - **右側（グレー背景）**: 解約情報
   - **ペアリングロジック**: 新規日以降の最も早い解約日を自動マッチング
   - 解約情報がない場合は「解約情報なし（継続中）」プレースホルダーを表示
   - 新規日の新しい順にソート

3. **単独の解約レコード（グレーヘッダー）**: ペアにならなかった解約のみの記録
   - 新規契約とマッチングできなかった解約レコード
   - 解約日の新しい順に表示

**グルーピングの利点:**
- **視覚的な関連性**: 入院→退院、新規→解約の流れが一目瞭然
- **サービス状況の把握**: 継続中のサービス（解約なし）が明確
- **データの整理**: 関連情報がまとまって表示され、スクロールが減少
- **時系列の理解**: 日付ベースのマッチングで正確なライフサイクル表示

**主な機能:**
- **自動ペアリング**: 日付ベースで関連レコードを自動グループ化
- **色分け表示**: 各レコードタイプを視覚的に区別（入院:赤、退院:緑、新規:青、解約:グレー）
- **横並びレイアウト**: デスクトップで左右に並べて表示、モバイルで縦積み
- **編集・削除機能**: 各レコードを個別に編集・削除可能
- **Kintone自動連携**: アプリ184（入退院）とアプリ197（新規変更）から毎日同期
- **手動入力対応**: 在宅利用者など、Kintoneにないケースも追加可能
...
| 項目名 | 入力形式 | 選択肢 |
|--------|----------|--------|
| 事業所選択 | テキスト（読み取り専用） | 基本情報タブから自動参照（鹿児島（ACG） / 福岡（Lichi）） |
| 記録者 | テキスト | 担当者名 |
...
| 技術 | バージョン | 用途 |
|------|-----------|------|
| **Google AI API** | Latest | Gemini API for browsers |
| **@google/generative-ai** | Latest | Browser-compatible Gemini SDK |
| **Gemini Model** | 2.0 Flash Experimental | 議事録生成・用具提案 |
| **認証方式** | API Key | ブラウザ互換・環境変数経由 |
...
interface Client {
  ...
  office: OfficeLocation        // 事業所 ('鹿児島（ACG）' | '福岡（Lichi）')
  name: string                  // 氏名
...
- [x] **Firestoreデータ永続化システム（2025-12-25実装）**
  ...
  - **福祉用具利用フラグ（2025-12-26実装）**
    - `isWelfareEquipmentUser`フラグをFirestoreに保存
    - 基本情報タブで手動チェック可能（介護保険・自費レンタル・販売すべて対応）
    - このフラグで福祉用具利用者のみをフィルタリング可能
    - Kintone/スプレッドシートからの取得データと手動設定を統合
- [x] **福祉用具マスターデータ連携（2025-12-25実装）**
  ...
  - **新規福祉用具追加時のデフォルト値**: 全てのフィールドが空白状態で追加される
