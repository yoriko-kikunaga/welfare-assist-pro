...
- **データ品質向上**（自動インポート統合済み）:
  ...
  - **重要**: importSpreadsheetData.cjsに統合され、1時間ごとの自動同期で継続的に更新
...
Cloud Schedulerにより自動実行されるベースデータのインポート:
...
被保険者証情報や福祉用具利用者情報など、必要時に手動でアップロードするデータ:

**2-1. 被保険者証情報のインポート:**

```bash
node importAdditionalData.cjs
```
